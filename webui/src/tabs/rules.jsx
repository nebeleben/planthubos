  ...
  return (
    <div>
      ...
      <div class="panel">
        ...
        {/* button.action's value is a press-code enum, and dim.rotate's is a
            signed CW/CCW turn -- neither fits in the compiler's own
            error/hint surface (no compiler change here), so it's a plain
            source-text sniff: any rule referencing either capability gets
            the decode hint right under the editor, regardless of where in
            the source it appears. */}
        {(source.includes('button.action') || source.includes('dim.rotate')) && (
          <p class="hint">
            Press codes: 1 single · 2 double · 3 triple · 0 hold · 255 release. Use edge mode, e.g.{' '}
            device("zb:...").button.action == 1. dim.rotate &gt; 0 is a clockwise turn, &lt; 0 counter-clockwise.
          </p>
        )}
        <p>

...
/* set/clear/lookup are called from the httpd task (API), prune/lookup from
 * the decoder task (dispatch/reindex) -- guard the RAM table with a spinlock
 * so a bind racing a reindex-prune can't silently drop the just-written
 * binding. Host build (test_wrapper_bind) is single-threaded, so these are
 * a no-op there. */
#ifdef ESP_PLATFORM
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
static portMUX_TYPE s_lock = portMUX_INITIALIZER_UNLOCKED;
#define WBIND_ENTER() taskENTER_CRITICAL(&s_lock)
#define WBIND_EXIT()  taskEXIT_CRITICAL(&s_lock)
#else
#define WBIND_ENTER() ((void)0)
#define WBIND_EXIT()  ((void)0)
#endif
...
bool wrapper_bind_set(const uint8_t mac[6], uint16_t wrapper_id) {
    ...
    if (wrapper_id != 0) {
        int i = find(mac);
        if (i < 0) { for (i = 0; i < WRAPPER_BIND_MAX && s_used[i]; i++) { } }
        if (i < WRAPPER_BIND_MAX) {
...
bool wrapper_bind_clear(const uint8_t mac[6]) {
    ...
    WBIND_ENTER();
    int i = find(mac);
    if (i >= 0) { s_used[i] = false; memset(&s_tbl[i], 0, sizeof s_tbl[i]); ok = true; }
    WBIND_EXIT();
    return ok;
}
...
uint16_t wrapper_bind_lookup(const uint8_t mac[6]) {
    WBIND_ENTER();
    int i = find(mac);
    uint16_t id = i < 0 ? 0 : s_tbl[i].wrapper_id;
    WBIND_EXIT();
    return id;
}
...
/* Takes the lock too: wrapper_bind_save() (ESP-only, below) calls this to
 * read s_tbl/s_used, and without it that read could tear against a
 * concurrent set/clear/prune. Safe to nest with deserialize's own lock
 * below since the two are never called from within each other. */
size_t wrapper_bind_serialize(uint8_t *buf, size_t cap) {
    ...
    for (int i = 0; i < WRAPPER_BIND_MAX; i++) {
        ...
        if (off + 8 > cap) { overflow = true; break; }
